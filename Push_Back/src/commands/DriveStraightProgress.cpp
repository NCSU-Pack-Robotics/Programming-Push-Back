#include "DriveStraightProgress.hpp"

#include "../math/Utils.hpp"

void DriveStraightProgress::initialize() {
    this->done = false;
...
void DriveStraightProgress::periodic() {
    printf("Progress: %.3f\n", this->get_progress());
    printf("Dist: %.3f\n", this->distance_driven);
    ...
    distance_driven = distance_driven_right;
...
void DriveStraightProgress::shutdown() {
    drivetrain.brake_now();
}

bool DriveStraightProgress::is_complete() {
    return this->done;
}

double DriveStraightProgress::get_progress() {
